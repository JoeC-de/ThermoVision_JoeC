...
namespace Khendys.Controls {
	...
	public class ExRichTextBox : System.Windows.Forms.RichTextBox {
		...
		public void InsertImage(Image _image) {
			...
			this.SelectedRtf = GetInsertImageCommand(_image);
		}
		public string GetInsertImageCommand(Image _image)
		{
			if (_image==null) { return ""; }
			StringBuilder _rtf = new StringBuilder();
		...
		#endregion
		public string getDatatable(DataGridView DGW,int randabstand) {
			StringBuilder _output = new StringBuilder();
			for (int i = 0; i <= DGW.Rows.Count; i++) {
				//anfang und abstand zum rand 100
				_output.Append(@"\trowd\trgaph100\trleft"+randabstand.ToString());
				//borders
				_output.Append(@"\clbrdrt\brdrw15\brdrs");
				_output.Append(@"\clbrdrl\brdrw15\brdrs");
				_output.Append(@"\clbrdrb\brdrw15\brdrs");
				_output.Append(@"\clbrdrr\brdrw15\brdrs ");
				//Spalten 1-3
				for (int C = 1; C < 4; C++) {
					_output.Append(@"\cellx"+(randabstand+(1000*C)).ToString());
					_output.Append(@"\clbrdrt\brdrw15\brdrs");
					_output.Append(@"\clbrdrl\brdrw15\brdrs");
					_output.Append(@"\clbrdrb\brdrw15\brdrs");
					_output.Append(@"\clbrdrr\brdrw15\brdrs ");
				}
				//ende des spaltendescriptors
				_output.Append(@"\pard\intbl\ltrpar\sl276\slmult1 ");
                if (i==0) {
                    _output.Append(@"\b Name\b0\cell ");
                    _output.Append(@"\b Label\b0\cell ");
                    _output.Append(@"\b Value\b0\cell ");
                    _output.Append(@"\row");
                    continue;
                }
				//12\cell\row
				_output.Append(Regex.Escape(DGW.Rows[i-1].Cells[0].Value.ToString())+@"\cell ");
				_output.Append(Regex.Escape(DGW.Rows[i-1].Cells[1].Value.ToString())+@"\cell ");
				_output.Append(Regex.Escape(Math.Round((float)DGW.Rows[i-1].Cells[2].Value, 1).ToString())+@"\cell ");
//				_output.Append(DGW.Rows[i].Cells[2].Value.ToString()+@"\cell ");
				_output.Append(@"\row");
				
			}
			return _output.ToString();
		}
		public void AppendDatatable(DataGridView DGW) {
			this.InsertTextAsRtf(getDatatable(DGW,720));
		}
	}
}
